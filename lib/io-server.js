...
export default (ioServer) => {
  ...
  ioServer.on('connection', socket => {
    ...
    socket.on('CREATE_ROOM', () => {
      ...
      let data = { 'roomCode': roomCode, 'maxPlayers': room.maxPlayers, 'roomHost': socket.id };
    ...
    socket.on('REDIRECT_PLAYERS', (roomCode, path) => {
      console.log('sending players to game', roomCode, path);
    ...
    socket.on('NEXT TURN', (data, roomCode) => {
      socket.broadcast.to(roomCode).emit('SWITCH TURNS', data);
    });

    socket.on('GAME WON', (roomCode, data) => {
      winner = data;
      ioServer.in(roomCode).emit('GAME OVER');
    });
