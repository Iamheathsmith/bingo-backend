...
export default (ioServer) => {
  ioServer.on('connection', socket => {
    ...
    // ==================== CREATE ROOM ==================== //
    socket.on('CREATE_ROOM', (instance) => {
    ...
    socket.on('JOIN_ROOM', (roomCode, nickname) => {
      ...
      if (room) {
        ...
        socket.emit('JOINED_ROOM', room.instance, room.maxPlayers);
    ...
    socket.on('START_GAME', data => {
      ...
      room.startGame(roomCode, socket, ioServer);
      log(`__GAME_STARTED__: [${roomCode}]`);
    });
